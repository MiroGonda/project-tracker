...
export const listBoards = (force = false) => {
  ...
  return phobosClient().get('/boards').then(r => {
...
export const boardSummary = (boardId) =>
  phobosClient().get(`/boards/${boardId}/summary`).then(r => r.data?.data)
...
export const boardCards = (boardId, params = {}) =>
  phobosClient().get(`/boards/${boardId}/cards`, { params })
    .then(r => {
      const raw = r.data?.data
...
export const boardMovements = (boardId, params = {}) =>
  phobosClient().get(`/boards/${boardId}/movements`, { params })
    .then(r => {
      const raw = r.data?.data
      return { data: Array.isArray(raw) ? raw : [], meta: r.data?.meta || {} }
    })
...
export const cycleTime = (rtProjectId, params = {}) =>
  phobosClient().get('/cycle-time', { params: { rtProjectId, ...params } })
    .then(r => {
      const raw = r.data?.data
      return { data: Array.isArray(raw) ? raw : [], meta: r.data?.meta || {} }
    })
